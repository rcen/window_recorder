    ...
    window.addEventListener('beforeunload', function(e) {
        ...
        if (hasUnsaved) {
            ...
            file.write(
                ...
                f'  <div id="work-timer-header" style="display:flex; align-items:center; justify-content:space-between; gap:10px; cursor:move; user-select:none;">\n'
                ...
                f'  let soundPlayed = streakSec >= thresholdSec;\n'
                f'\n'
                f'  function playNotificationSound() {{\n'
                f'    try {{\n'
                f'      const audioCtx = new (window.AudioContext || window.webkitAudioContext)();\n'
                f'      const playTone = (freq, startTime, duration) => {{\n'
                f'        const osc = audioCtx.createOscillator();\n'
                f'        const gain = audioCtx.createGain();\n'
                f'        osc.type = "sine";\n'
                f'        osc.frequency.setValueAtTime(freq, startTime);\n'
                f'        gain.gain.setValueAtTime(0.15, startTime);\n'
                f'        gain.gain.exponentialRampToValueAtTime(0.0001, startTime + duration);\n'
                f'        osc.connect(gain);\n'
                f'        gain.connect(audioCtx.destination);\n'
                f'        osc.start(startTime);\n'
                f'        osc.stop(startTime + duration);\n'
                f'      }};\n'
                f'      const now = audioCtx.currentTime;\n'
                f'      playTone(880, now, 0.4);\n'
                f'      playTone(1109, now + 0.15, 0.55);\n'
                f'    }} catch (e) {{\n'
                f'      console.error("Failed to play notification sound:", e);\n'
                f'    }}\n'
                f'  }}\n'
                f'\n'
                f'  // Draggable logic\n'
                f'  const header = document.getElementById("work-timer-header");\n'
                f'  if (header) {{\n'
                f'    let isDragging = false;\n'
                f'    let startX, startY, initialLeft, initialTop;\n'
                f'\n'
                f'    header.addEventListener("mousedown", (e) => {{\n'
                f'      if (e.target.id === "work-timer-close") return;\n'
                f'      isDragging = true;\n'
                f'      startX = e.clientX;\n'
                f'      startY = e.clientY;\n'
                f'\n'
                f'      const rect = popup.getBoundingClientRect();\n'
                f'      initialLeft = rect.left;\n'
                f'      initialTop = rect.top;\n'
                f'\n'
                f'      popup.style.right = "auto";\n'
                f'      popup.style.bottom = "auto";\n'
                f'      popup.style.left = initialLeft + "px";\n'
                f'      popup.style.top = initialTop + "px";\n'
                f'\n'
                f'      document.addEventListener("mousemove", onMouseMove);\n'
                f'      document.addEventListener("mouseup", onMouseUp);\n'
                f'      e.preventDefault();\n'
                f'    }});\n'
                f'\n'
                f'    function onMouseMove(e) {{\n'
                f'      if (!isDragging) return;\n'
                f'      const dx = e.clientX - startX;\n'
                f'      const dy = e.clientY - startY;\n'
                f'      popup.style.left = (initialLeft + dx) + "px";\n'
                f'      popup.style.top = (initialTop + dy) + "px";\n'
                f'    }}\n'
                f'\n'
                f'    function onMouseUp() {{\n'
                f'      isDragging = false;\n'
                f'      document.removeEventListener("mousemove", onMouseMove);\n'
                f'      document.removeEventListener("mouseup", onMouseUp);\n'
                f'    }}\n'
                f'  }}\n'
                f'\n'
                ...
                f'    popup.style.display = "none";\n'
                f'    if (fab && enabled && thresholdSec > 0) fab.style.display = "block";\n'
                f'  }});\n'
                f'\n'
                ...
                f'    if (barEl) {{\n'
                f'      barEl.style.width = pct.toString() + "%";\n'
                f'      if (streakSec >= thresholdSec) {{\n'
                f'        barEl.style.background = "linear-gradient(90deg, #ffca28, #ff8f00)";\n'
                f'      }} else {{\n'
                f'        barEl.style.background = "linear-gradient(90deg, #66bb6a, #2e7d32)";\n'
                f'      }}\n'
                f'    }}\n'
                f'    if (streakSec >= thresholdSec) {{\n'
                f'      popup.style.border = "1px solid rgba(255, 215, 0, 0.4)";\n'
                f'      popup.style.borderLeft = "6px solid #ffd700";\n'
                f'      if (doneEl) doneEl.style.display = "block";\n'
                f'    }} else {{\n'
                f'      popup.style.border = "1px solid rgba(76, 175, 80, 0.35)";\n'
                f'      popup.style.borderLeft = "6px solid #4caf50";\n'
                f'      if (doneEl) doneEl.style.display = "none";\n'
                f'    }}\n'
                f'  }}\n'
                f'\n'
                f'  // Show if enabled and not dismissed\n'
                f'  if (!enabled || dismissed || thresholdSec <= 0) {{\n'
                f'    popup.style.display = "none";\n'
                f'    if (fab) {{\n'
                f'      if (enabled && dismissed && thresholdSec > 0) fab.style.display = "block";\n'
